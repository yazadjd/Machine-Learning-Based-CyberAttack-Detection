...
The below code uses the iForest algorithm for performing botnet detection through unsupervised learning on the 3 separately generated feature sets.
...
X_test3.drop(['Duration_max',	'Duration_median', 'BiDirection_Bytes_median', 'SrcToDst_Bytes_median'], inplace = True, axis = 1) #Dropping due to high correlation as calculated previously
...
X_valid3.drop(['Duration_max',	'Duration_median', 'BiDirection_Bytes_median', 'SrcToDst_Bytes_median', 'Label_<lambda>'], inplace = True, axis = 1) #Dropping due to high correlation as calculated previously
...
"""Experiment with Tuned value of Contamination similar to above"""

clf1 = IForest(verbose = 1, random_state = 42, contamination=0.35) 
clf1.fit(X_train1)
...
# Test data is scored
y_test_scores = clf1.decision_function(X_test1)
...
extracted_rows.to_csv('IFOREST_Set1.csv', index = True) # Write to csv
...
clf1 = IForest(random_state = 42) #Contamination = 0.1
clf1.fit(X_train2)
...
extracted_rows.drop(labels=['Direction', 'State', 'Source_Service',	'Dest_Service', 'Total_Packets', 'BiDirection_Bytes',	'SrcToDst_Bytes'], inplace=True, axis=1)
extracted_rows.to_csv('IFOREST_Set2.csv', index = True)
...
clf1 = IForest(verbose = 1, random_state = 42, contamination=0.20) 
clf1.fit(X_train3)
...
extracted_rows.to_csv('IFOREST_Set3.csv', index = True)
